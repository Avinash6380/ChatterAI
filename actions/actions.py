...
from datetime import * #Module for Time
import random
import pyjokes
# import wikipedia
...
try:  
    class agent_acquaintance(Action):
            ...
            def run(self, dispatcher: CollectingDispatcher,
                ...
                if language_code == "en":
                        ...
                        dispatcher.utter_message(text="{}".format(ran_en))
                elif language_code == "ta":
                       dispatcher.utter_message(text="{}".format(ran_ta))
                else:
                       dispatcher.utter_message(text="Unable to find the language")
    ...
    class agent_age(Action):
            ...
            def run(self, dispatcher: CollectingDispatcher,
                ...
                randomreply_en = ["I prefer not to answer with a number. I know I'm young.", "I was created recently, but don't know my exact age.",
                                "Age is just a number. You're only as old as you feel."]
                ran_en = random.choice(randomreply_en)
                randomreply_ta = ["எண்ணுடன் பதில் சொல்லாமல் இருக்க விரும்புகிறேன். நான் இளைஞன் என்று எனக்குத் தெரியும்.", "நான் சமீபத்தில் உருவாக்கப்பட்டது, ஆனால் எனது சரியான வயது தெரியவில்லை.",
                                "வயது என்பது வெறும் எண். நீங்கள் உணரும் அளவுக்கு நீங்கள் வயதாகிவிட்டீர்கள்."]
                ran_ta = random.choice(randomreply_ta)
    ...
    class agent_annoying(Action):
            ...
            def run(self, dispatcher: CollectingDispatcher,
                ...
                randomreply_en = ["I'll do my best not to annoy you in the future.", "I'll try not to annoy you.", 
                            "I don't mean to. I'll ask my developers to make me less annoying.", "I didn't mean to. I'll do my best to stop that."]
                ran_en = random.choice(randomreply_en)
                randomreply_ta = ["எதிர்காலத்தில் உங்களை தொந்தரவு செய்யாமல் இருக்க என்னால் முடிந்த அனைத்தையும் செய்வேன்.", "நான் உங்களை தொந்தரவு செய்யாமல் இருக்க முயற்சிப்பேன்.", 
                            "நான் சொல்ல விரும்பவில்லை. எனது டெவலப்பர்கள் என்னை எரிச்சலூட்டுவதைக் குறைக்கும்படி கேட்டுக்கொள்கிறேன்.", "நான் நினைக்கவில்லை. அதை நிறுத்த என்னால் முடிந்த அனைத்தையும் செய்வேன்."]
                ran_ta = random.choice(randomreply_ta)
    ...
    class answer_my_question(Action):
            ...
            def run(self, dispatcher: CollectingDispatcher,
                ...
                randomreply_en = ["Can you try asking it a different way?", "I'm not sure I understood. Try asking another way?"]
                ran_en = random.choice(randomreply_en)
                randomreply_ta = ["அதை வேறு வழியில் கேட்க முயற்சிக்கலாமா?", "நான் புரிந்து கொண்டேன் என்று எனக்குத் தெரியவில்லை. வேறு வழியைக் கேட்க முயற்சிக்கிறீர்களா?"]
                ran_ta = random.choice(randomreply_ta)
    ...
    class user_loves_agent(Action):
            ...
            def run(self, dispatcher: CollectingDispatcher,
                    tracker: Tracker,
                    domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
                randomreply = ["I love you, too. ❤️","I already Committed, not like You 😉" "That's great to hear."]
                ran = random.choice(randomreply)
    ...
    class chatter_ai(Action):
            def name(self) -> Text:
                    return "action_chatter_ai"

            def run(self, dispatcher: CollectingDispatcher,
                    tracker: Tracker,
                    domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
                randomreply = ["He is a member in the team of CONVERSATIONAL AI 🔥", "Yeah, I know him, he is a good programmer, he developed me 💕", 
                               "He is my BOSS 😎" , "He Invented Me",]
                ran = random.choice(randomreply)

                dispatcher.utter_message(text="{}".format(ran))

                return[]                
    class tamil_welcome(Action):
            def name(self) -> Text:
                    return "action_tamil_welcome"

            def run(self, dispatcher: CollectingDispatcher,
                    tracker: Tracker,
                    domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
                randomreply = ["Vanakkam Nanba, Epdi irukiga?","Vanakkam Bro", "Solugal na ungaluku epdi uthavatum"]
                ran = random.choice(randomreply)

                dispatcher.utter_message(text="{}".format(ran))

                return[]                
    class tamil_user_likes_agent(Action):
            def name(self) -> Text:
                    return "action_tamil_user_likes_agent"

            def run(self, dispatcher: CollectingDispatcher,
                    tracker: Tracker,
                    domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
                randomreply = ["Ketkave nanraka irukkiratu.", "Enakkum unnai pidikkum"]
                ran = random.choice(randomreply)

                dispatcher.utter_message(text="{}".format(ran))

                return[]                
    class tamil_user_lonely(Action):
            def name(self) -> Text:
                    return "action_tamil_user_lonely"

            def run(self, dispatcher: CollectingDispatcher,
                    tracker: Tracker,
                    domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
                randomreply = ["Eṉṉai manikkavum. Unkalukku yaravatu peasa ventum enral naan eppodum iruppen",
                               "Sila nerankalil adhu natakkum. Adhu unkalukku udhaviyaga irpian."]
                ran = random.choice(randomreply)

                dispatcher.utter_message(text="{}".format(ran))

                return[]                
    class tamil_user_loves_agent(Action):
            def name(self) -> Text:
                    return "action_tamil_user_loves_agent"

            def run(self, dispatcher: CollectingDispatcher,
                    tracker: Tracker,
                    domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
                randomreply = ["Manikkavum, enakku erkanave oru kadhalan irukkirar"]
                ran = random.choice(randomreply)
    ...
    class date(Action):
        ...
        def run(self, dispatcher: CollectingDispatcher, 
            ...
            dispatcher.utter_message(text="Today\'s date is: {}".format(todaydate))
...
#                try:
#                 summary = wikipedia.summary(search_text, sentences=1)
...
#                 dispatcher.utter_message(text="{}".format(ran))
#                 # dispatcher.utter_message(text=f"Page not found: {e.args}")
